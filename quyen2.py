        ...
        if st.session_state.search_selection == "top_courses":
            ...
            for index, row in enumerate(top_courses.iterrows()):
                ...
                with cols[index % 3]:
                    ...
                    """, unsafe_allow_html=True)
                    button_key = f"like_{row['CourseID']}"
                    if row['CourseName'] in st.session_state.selected_courses:
                        st.success("You liked this course")
                    elif st.button("Like This Course", key=button_key):
